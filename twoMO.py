...
def parEI(gp1, gp2, X_sample, Y_sample, EI=True, KG=False, truth=False):
    x = np.linspace(XL, XU, 100)
    ins = (x)
    #ins = np.atleast_2d(x).T
    #evs = gp.predict(ins)
    if KG:
       eis = [KG(xc) for xc in x]
       eis2 = [KG(xc) for xc in x]
...
for __ in range(120):
   ...
   if True:
      ...
      ax[0][1].plot(x, s, label='EI')
      ax[0][1].plot(x, s2, label='EI', ls='--')
      #ax[1].plot(x, np.max([s, np.zeros(len(s))], 0), label='EI')
      #ax[1].plot(x, np.max([s2, np.zeros(len(s))], 0), label='NEI', ls='--')
      ax[0][1].plot(x, np.max([np.sqrt(np.array(s) ** 2 + np.array(s2) ** 2), np.zeros(len(s))], 0), label='NEI', ls='-.')
      ...
      ax[1][0].scatter(b.T[:, 0], b.T[:, 1], c=np.sqrt((b.T[:, 0] ** 2 +  b.T[:, 1] ** 2)))
