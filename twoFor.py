...
for __ in range(120):
   ...
   def KG(x, expect=True):
      NROUNDS = 100
      samps = np.random.normal(0, gpf(x, return_std=True)[1], NROUNDS)
      mupf = gpf(x)[0]
      summ = 0
      for ll in range(NROUNDS):
         points = [s[0] for s in list(thesePoints) + [np.array(x)]]
         evals = [s[0] for s in list(theseEvals) + [mupf + samps[ll]]]
         ...
         if expect:
            term = (expected_improvement(min_x, np.array(points), np.array(evals), gpf)[0] - expected_improvement(min_x, np.array(points), np.array(evals), gpf_next)[0])
         else:
            term = (gpf(min_x)[0] - gpf_next(min_next_x))[0]
         #print(ll, term)
         summ += term
      return summ / NROUNDS
