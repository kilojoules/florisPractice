...
for __ in range(20000):
   ...
   if PLOT:
      ...
      #ax12 = ax[0, 0].twinx()
      ax[0, 1].fill_between(xx, pd2 - sd2, pd2 + sd2, facecolor='red', alpha=0.7)
      ...
      ax[0, 1].plot(xx, pd2, c='red')
      ax[0, 1].plot(xx, [f(np.array([xc]), lf=False)[1] for xc in xx], c='yellow', ls='-')
      ax[0, 1].scatter(x1, fHs2, c='w', marker='x')
      ax[1, 0].set_xlabel('x')
      ...
      ax[0, 0].annotate(r'$l_{\mathrm{Power}}=%.2f$' % gpdelta.kernel_.get_params()['length_scale'], [-25, -1.5])
      ax[0, 1].annotate(r'$l_{\mathrm{Loading}}=%.2f$' % gpdelta2.kernel_.get_params()['length_scale'], [-20, -9.75])
      ...
      ax[1, 1].set_ylabel('Loading')
      plt.suptitle('%i Samples' % (__ + 2))
